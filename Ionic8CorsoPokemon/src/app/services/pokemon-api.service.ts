...
export class PokemonApiService {
  ...
  async init() {
    // If using, define drivers here: await this.storage.defineDriver(customDriver)
  ...
  async addPokemonFavorite(pok:IPokemon): Promise<boolean>{
    try {
      const res = await this._storage?.get('POKEMON_FAVORITE');
      ...
      data.push(pok);
      await this._storage?.set('POKEMON_FAVORITE',data);
  ...
  async removePokemonFavorite(pok:IPokemon):Promise<IPokemon[]>{
    await this._storage?.get('POKEMON_FAVORITE').then( (res) => {
      let data:IPokemon[]=res ?? [];
      data=data.filter(r => r.name!==pok.name);
      this._storage?.set('POKEMON_FAVORITE',data);
    });
    return this.getFavoritesPokemon();
  }
  async getFavoritesPokemon():Promise<IPokemon[]>{
    return (await this._storage?.get('POKEMON_FAVORITE')) || [];
  }
  async isPokemonInFavorites(pok:IPokemon):Promise<Boolean>{
    let data:IPokemon[] = (await this._storage?.get('POKEMON_FAVORITE')) || [];
    if (data.length == 0)
      return false;
    if (data.some(e => e.name === pok.name))
      return true;
